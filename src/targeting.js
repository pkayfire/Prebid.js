...
export function newTargeting(auctionManager) {
  ...
  function getWinningBidsWithSharing(originalWinningBids, adUnitCodes, bidsReceived) {
    ...
    const reassignedBidTotal = Object.values(winningBidAssignments).reduce((acc, next) => acc + next.cpm, 0);
    const gainAmount = reassignedBidTotal - originalTotal;
    const gainPercentage = gainAmount / originalTotal;
    const hasEnoughGain = gainAmount > 0.1 && gainPercentage > 0.01;

    utils.logMessage('Change Summary', changeSummaries);
  ...
  targeting.getWinningBids = function(adUnitCode, bidsReceived = getBidsReceived()) {
    ...
    return getWinningBidsWithSharing(originalWinningBids, adUnitCodesWithBids, bidsReceived);
  };
